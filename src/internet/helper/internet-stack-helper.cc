...
/*
void
InternetStackHelper::InstallNetDevice (Ptr<NetDevice> netDevice) const
{
  if (m_ipv4Enabled)
    {
      if (netDevice->GetObject<Ipv4> () != 0)
        {
          NS_FATAL_ERROR ("InternetStackHelper::Install (): Aggregating " 
                          "an InternetStack to a node with an existing Ipv4 object");
          return;
      ...
      CreateAndAggregateObjectFromTypeId (netDevice, "ns3::ArpL3Protocol");
      CreateAndAggregateObjectFromTypeId (netDevice, "ns3::Ipv4L3Protocol");
      CreateAndAggregateObjectFromTypeId (netDevice, "ns3::Icmpv4L4Protocol");
      if (m_ipv4ArpJitterEnabled == false)
        {
          Ptr<ArpL3Protocol> arp = netDevice->GetObject<ArpL3Protocol> ();
          NS_ASSERT (arp);
          arp->SetAttribute ("RequestJitter", StringValue ("ns3::ConstantRandomVariable[Constant=0.0]"));
        }
      // Set routing
      Ptr<Ipv4> ipv4 = netDevice->GetObject<Ipv4> ();
      Ptr<Ipv4RoutingProtocol> ipv4Routing = m_routing->CreateNetDevice (netDevice);
      ipv4->SetRoutingProtocol (ipv4Routing);
    }
  ...
  if (m_ipv6Enabled)
    {
      ...
      CreateAndAggregateObjectFromTypeId (netDevice, "ns3::Ipv6L3Protocol");
      CreateAndAggregateObjectFromTypeId (netDevice, "ns3::Icmpv6L4Protocol");
      if (m_ipv6NsRsJitterEnabled == false)
        {
          Ptr<Icmpv6L4Protocol> icmpv6l4 = netDevice->GetObject<Icmpv6L4Protocol> ();
          NS_ASSERT (icmpv6l4);
          icmpv6l4->SetAttribute ("SolicitationJitter", StringValue ("ns3::ConstantRandomVariable[Constant=0.0]"));
        }
      // Set routing
      Ptr<Ipv6> ipv6 = netDevice->GetObject<Ipv6> ();
  ...
  if (m_ipv4Enabled || m_ipv6Enabled)
    {
      CreateAndAggregateObjectFromTypeId (netDevice, "ns3::TrafficControlLayer");
      CreateAndAggregateObjectFromTypeId (netDevice, "ns3::UdpL4Protocol");
      netDevice->AggregateObject (m_tcpFactory.Create<Object> ());
      Ptr<PacketSocketFactory> factory = CreateObject<PacketSocketFactory> ();
      netDevice->AggregateObject (factory);
    }

  if (m_ipv4Enabled)
    {
      Ptr<ArpL3Protocol> arp = netDevice->GetObject<ArpL3Protocol> ();
      Ptr<TrafficControlLayer> tc = netDevice->GetObject<TrafficControlLayer> ();
      NS_ASSERT (arp);
      NS_ASSERT (tc);
      arp->SetTrafficControl (tc);
    }
