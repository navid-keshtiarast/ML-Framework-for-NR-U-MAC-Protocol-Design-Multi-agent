...
ThreeGppRmaPropagationLossModel::GetLossLos (double distance2D, double distance3D, double hUt, double hBs) const
{
  ...
  if (distance2D <= distanceBp)
    ...
    {
      // use PL2
      loss =Pl1 (m_frequency, distance3D, m_h, m_w);//Pl1 (m_frequency, distanceBp, m_h, m_w) + 40 * log10 (distance3D / distanceBp); Pl1 (m_frequency, distanceBp, m_h, m_w) + 40 * log10 (distance3D / distanceBp);navid
    }
...
ThreeGppRmaPropagationLossModel::GetLossNlos (double distance2D, double distance3D, double hUt, double hBs) const
{
  ...
  double plNlos = 0;//161.04 - 7.1 * log10 (m_w) + 7.5 * log10 (m_h) - (24.37 - 3.7 * pow ((m_h / hBs), 2)) * log10 (hBs) + (43.42 - 3.1 * log10 (hBs)) * (log10 (distance3D) - 3.0) + 20.0 * log10 (m_frequency / 1e9) - (3.2 * pow (log10 (11.75 * hUt), 2) - 4.97);Navid
  ...
  if (cond == ChannelCondition::LosConditionValue::LOS)
    {
      ...
      // if (distance2d <= distanceBp)
      //   {
      //     shadowingStd = 0.0;///4.0;Navid
      //   }
      // else
      //   {
      //     shadowingStd = 0.0;///6.0;Navid
      //   }
        shadowingStd = 0.0;
    }
  else if (cond == ChannelCondition::LosConditionValue::NLOS)
    {
      shadowingStd =0.0;// 8.0;
    }
  else
    {
      NS_FATAL_ERROR ("Unknown channel condition");
...
{
  //NS_UNUSED (w);
  double loss =0;// 20.0 * log10 (40.0 * M_PI * distance3D * frequency / 1e9 / 3.0) + std::min (0.03 * pow (h, 1.72), 10.0) * log10 (distance3D) - std::min (0.044 * pow (h, 1.72), 14.77) + 0.002 * log10 (h) * distance3D;
  return loss;
}
