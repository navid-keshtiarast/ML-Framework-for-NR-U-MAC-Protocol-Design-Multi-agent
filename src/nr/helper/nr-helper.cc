...
NetDeviceContainer
NrHelper::InstallMultipleGnbDevice (const NodeContainer &c,
                               const std::vector<std::reference_wrapper<BandwidthPartInfoPtr> > &allBwps)
{
  NS_LOG_FUNCTION (this);
  Initialize ();    // Run DoInitialize (), if necessary
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      ...
      Ptr<NrGnbNetDevice> dev1 = m_gnbNetDeviceFactory.Create<NrGnbNetDevice> ();
      Ptr<NrGnbNetDevice> dev2 = m_gnbNetDeviceFactory.Create<NrGnbNetDevice> ();
      Ptr<NrGnbNetDevice> dev3 = m_gnbNetDeviceFactory.Create<NrGnbNetDevice> ();

      NS_LOG_DEBUG ("Creating gNB, cellId = " << m_cellIdCounter);
      uint16_t cellId = m_cellIdCounter++;

      dev1->SetCellId (cellId);
      dev1->SetNode (node);
      dev2->SetCellId (cellId);
      dev2->SetNode (node);
      dev3->SetCellId (cellId);
      dev3->SetNode (node);

      std::map<uint8_t,Ptr<BandwidthPartGnb> > ccMap1;
      std::map<uint8_t,Ptr<BandwidthPartGnb> > ccMap2;
      std::map<uint8_t,Ptr<BandwidthPartGnb> > ccMap3;
      ...
      for (uint32_t bwpId = 0; bwpId < allBwps.size (); ++bwpId)
      {
        ...
        SetGnbChannelAccessManagerTypeId (TypeId::LookupByName("ns3::NrCat4LbtAccessManager"));
        auto phy = CreateGnbPhy (node, allBwps[bwpId].get(), dev1,
                                std::bind (&NrGnbNetDevice::RouteIngoingCtrlMsgs,
                                            dev1, std::placeholders::_1, bwpId));
        phy->SetBwpId (bwpId);
        cc->SetPhy (phy);
        ...
        ccMap1.insert (std::make_pair (bwpId, cc));
      }
      ...
      for (uint32_t bwpId = 0; bwpId < allBwps.size (); ++bwpId)
      {
        ...
        SetGnbChannelAccessManagerTypeId (TypeId::LookupByName("ns3::NrCat2LbtAccessManager"));
        auto phy = CreateGnbPhy (node, allBwps[bwpId].get(), dev2,
                                std::bind (&NrGnbNetDevice::RouteIngoingCtrlMsgs,
                                            dev2, std::placeholders::_1, bwpId));
        phy->SetBwpId (bwpId);
        cc->SetPhy (phy);
        ...
        ccMap2.insert (std::make_pair (bwpId, cc));
      }
      ...
      for (uint32_t bwpId = 0; bwpId < allBwps.size (); ++bwpId)
      {
        ...
        SetGnbChannelAccessManagerTypeId (TypeId::LookupByName("ns3::NrCat2LbtAccessManager"));
        auto phy = CreateGnbPhy (node, allBwps[bwpId].get(), dev3,
                                std::bind (&NrGnbNetDevice::RouteIngoingCtrlMsgs,
                                            dev3, std::placeholders::_1, bwpId));
        phy->SetBwpId (bwpId);
        cc->SetPhy (phy);
        ...
        ccMap3.insert (std::make_pair (bwpId, cc));
      }

      Ptr<LteEnbRrc> rrc1 = CreateObject<LteEnbRrc> ();
      Ptr<LteEnbComponentCarrierManager> ccmEnbManager1 = DynamicCast<LteEnbComponentCarrierManager> (CreateObject<BwpManagerGnb> ());
      DynamicCast<BwpManagerGnb> (ccmEnbManager1)->SetBwpManagerAlgorithm (m_gnbBwpManagerAlgoFactory.Create <BwpManagerAlgorithm> ());
      ...
      std::map < uint8_t, Ptr<ComponentCarrierBaseStation> > ccPhyConfMap1;
      for (const auto &i : ccMap1)
        {
          Ptr<ComponentCarrierBaseStation> c = i.second;
          ccPhyConfMap1.insert (std::make_pair (i.first,c));
        }

      //ComponentCarrierManager SAP
      rrc1->SetLteCcmRrcSapProvider (ccmEnbManager1->GetLteCcmRrcSapProvider ());
      ccmEnbManager1->SetLteCcmRrcSapUser (rrc1->GetLteCcmRrcSapUser ());
      // Set number of component carriers. Note: eNB CCM would also set the
      // number of component carriers in eNB RRC

      ccmEnbManager1->SetNumberOfComponentCarriers (ccMap1.size ());
      rrc1->ConfigureCarriers (ccPhyConfMap1);
      ...
      if (useIdealRrc)
        {
          ...
          rrcProtocol->SetLteEnbRrcSapProvider (rrc1->GetLteEnbRrcSapProvider ());
          rrc1->SetLteEnbRrcSapUser (rrcProtocol->GetLteEnbRrcSapUser ());
          rrc1->AggregateObject (rrcProtocol);
        }
      else
        {
      ...
      if (m_epcHelper != nullptr)
        {
          ...
          rrc1->GetAttribute ("EpsBearerToRlcMapping", epsBearerToRlcMapping);
          // it does not make sense to use RLC/SM when also using the EPC
          if (epsBearerToRlcMapping.Get () == LteEnbRrc::RLC_SM_ALWAYS)
            {
              rrc1->SetAttribute ("EpsBearerToRlcMapping", EnumValue (LteEnbRrc::RLC_UM_ALWAYS));
            }
        }
      ...
      rrc1->SetLteMacSapProvider (ccmEnbManager1->GetLteMacSapProvider ());
      rrc1->SetForwardUpCallback (MakeCallback (&NrGnbNetDevice::Receive, dev1));

      for (auto it = ccMap1.begin (); it != ccMap1.end (); ++it)
        {
          it->second->GetPhy ()->SetEnbCphySapUser (rrc1->GetLteEnbCphySapUser (it->first));
          rrc1->SetLteEnbCphySapProvider (it->second->GetPhy ()->GetEnbCphySapProvider (), it->first);

          rrc1->SetLteEnbCmacSapProvider (it->second->GetMac ()->GetEnbCmacSapProvider (),it->first );
          it->second->GetMac ()->SetEnbCmacSapUser (rrc1->GetLteEnbCmacSapUser (it->first));
          ...
          it->second->GetMac ()->SetLteCcmMacSapUser (ccmEnbManager1->GetLteCcmMacSapUser ());
          ccmEnbManager1->SetCcmMacSapProviders (it->first, it->second->GetMac ()->GetLteCcmMacSapProvider ());

          // insert the pointer to the LteMacSapProvider interface of the MAC layer of the specific component carrier
          ccmEnbManager1->SetMacSapProvider (it->first, it->second->GetMac ()->GetMacSapProvider ());
        }


      dev1->SetAttribute ("LteEnbComponentCarrierManager", PointerValue (ccmEnbManager1));
      dev1->SetCcMap (ccMap1);
      dev1->SetAttribute ("LteEnbRrc", PointerValue (rrc1));
      dev1->Initialize ();
      ...
      if (m_epcHelper != nullptr)
        {
          ...
          m_epcHelper->AddEnb (node, dev1, dev1->GetCellId ());
          Ptr<EpcEnbApplication> enbApp = node->GetApplication (0)->GetObject<EpcEnbApplication> ();
          NS_ASSERT_MSG (enbApp != nullptr, "cannot retrieve EpcEnbApplication");

          // S1 SAPs
          rrc1->SetS1SapProvider (enbApp->GetS1SapProvider ());
          enbApp->SetS1SapUser (rrc1->GetS1SapUser ());

          // X2 SAPs
          Ptr<EpcX2> x2 = node->GetObject<EpcX2> ();
          x2->SetEpcX2SapUser (rrc1->GetEpcX2SapUser ());
          rrc1->SetEpcX2SapProvider (x2->GetEpcX2SapProvider ());
        }
      
      Ptr<LteEnbRrc> rrc2 = CreateObject<LteEnbRrc> ();
      Ptr<LteEnbComponentCarrierManager> ccmEnbManager2 = DynamicCast<LteEnbComponentCarrierManager> (CreateObject<BwpManagerGnb> ());
      DynamicCast<BwpManagerGnb> (ccmEnbManager2)->SetBwpManagerAlgorithm (m_gnbBwpManagerAlgoFactory.Create <BwpManagerAlgorithm> ());

      // Convert Enb carrier map to only PhyConf map
      // we want to make RRC to be generic, to be able to work with any type of carriers, not only strictly LTE carriers
      std::map < uint8_t, Ptr<ComponentCarrierBaseStation> > ccPhyConfMap2;
      for (const auto &i : ccMap2)
        {
          Ptr<ComponentCarrierBaseStation> c = i.second;
          ccPhyConfMap2.insert (std::make_pair (i.first,c));
        }

      //ComponentCarrierManager SAP
      rrc2->SetLteCcmRrcSapProvider (ccmEnbManager2->GetLteCcmRrcSapProvider ());
      ccmEnbManager2->SetLteCcmRrcSapUser (rrc2->GetLteCcmRrcSapUser ());
      // Set number of component carriers. Note: eNB CCM would also set the
      // number of component carriers in eNB RRC

      ccmEnbManager2->SetNumberOfComponentCarriers (ccMap2.size ());
      rrc2->ConfigureCarriers (ccPhyConfMap2);

      if (useIdealRrc)
        {
          Ptr<NrGnbRrcProtocolIdeal> rrcProtocol = CreateObject<NrGnbRrcProtocolIdeal> ();
          rrcProtocol->SetLteEnbRrcSapProvider (rrc2->GetLteEnbRrcSapProvider ());
          rrc2->SetLteEnbRrcSapUser (rrcProtocol->GetLteEnbRrcSapUser ());
          rrc2->AggregateObject (rrcProtocol);
        }
      else
        {
          Ptr<LteEnbRrcProtocolReal> rrcProtocol = CreateObject<LteEnbRrcProtocolReal> ();
      ...
      if (m_epcHelper != nullptr)
        {
          EnumValue epsBearerToRlcMapping;
          rrc2->GetAttribute ("EpsBearerToRlcMapping", epsBearerToRlcMapping);
          // it does not make sense to use RLC/SM when also using the EPC
          if (epsBearerToRlcMapping.Get () == LteEnbRrc::RLC_SM_ALWAYS)
            {
              rrc2->SetAttribute ("EpsBearerToRlcMapping", EnumValue (LteEnbRrc::RLC_UM_ALWAYS));
            }
        }
      ...
      // forward these calls to the specific MAC of some of the instances of component carriers. This
      // decision will depend on the specific implementation of the component carrier manager.
      rrc2->SetLteMacSapProvider (ccmEnbManager2->GetLteMacSapProvider ());
      rrc2->SetForwardUpCallback (MakeCallback (&NrGnbNetDevice::Receive, dev2));

      for (auto it = ccMap2.begin (); it != ccMap2.end (); ++it)
        {
          it->second->GetPhy ()->SetEnbCphySapUser (rrc2->GetLteEnbCphySapUser (it->first));
          rrc2->SetLteEnbCphySapProvider (it->second->GetPhy ()->GetEnbCphySapProvider (), it->first);

          rrc2->SetLteEnbCmacSapProvider (it->second->GetMac ()->GetEnbCmacSapProvider (),it->first );
          it->second->GetMac ()->SetEnbCmacSapUser (rrc2->GetLteEnbCmacSapUser (it->first));
          ...
          it->second->GetMac ()->SetLteCcmMacSapUser (ccmEnbManager2->GetLteCcmMacSapUser ());
          ccmEnbManager2->SetCcmMacSapProviders (it->first, it->second->GetMac ()->GetLteCcmMacSapProvider ());

          // insert the pointer to the LteMacSapProvider interface of the MAC layer of the specific component carrier
          ccmEnbManager2->SetMacSapProvider (it->first, it->second->GetMac ()->GetMacSapProvider ());
        }


      dev2->SetAttribute ("LteEnbComponentCarrierManager", PointerValue (ccmEnbManager2));
      dev2->SetCcMap (ccMap2);
      dev2->SetAttribute ("LteEnbRrc", PointerValue (rrc2));
      dev2->Initialize ();
      ...
      if (m_epcHelper != nullptr)
        {
          NS_LOG_INFO ("adding this eNB to the EPC");
          m_epcHelper->AddEnb (node, dev2, dev2->GetCellId ());
          Ptr<EpcEnbApplication> enbApp = node->GetApplication (0)->GetObject<EpcEnbApplication> ();
          NS_ASSERT_MSG (enbApp != nullptr, "cannot retrieve EpcEnbApplication");

          // S1 SAPs
          rrc2->SetS1SapProvider (enbApp->GetS1SapProvider ());
          enbApp->SetS1SapUser (rrc2->GetS1SapUser ());

          // X2 SAPs
          Ptr<EpcX2> x2 = node->GetObject<EpcX2> ();
          x2->SetEpcX2SapUser (rrc2->GetEpcX2SapUser ());
          rrc2->SetEpcX2SapProvider (x2->GetEpcX2SapProvider ());
        }
      
      Ptr<LteEnbRrc> rrc3 = CreateObject<LteEnbRrc> ();
      Ptr<LteEnbComponentCarrierManager> ccmEnbManager3 = DynamicCast<LteEnbComponentCarrierManager> (CreateObject<BwpManagerGnb> ());
      DynamicCast<BwpManagerGnb> (ccmEnbManager3)->SetBwpManagerAlgorithm (m_gnbBwpManagerAlgoFactory.Create <BwpManagerAlgorithm> ());

      // Convert Enb carrier map to only PhyConf map
      // we want to make RRC to be generic, to be able to work with any type of carriers, not only strictly LTE carriers
      std::map < uint8_t, Ptr<ComponentCarrierBaseStation> > ccPhyConfMap3;
      for (const auto &i : ccMap3)
        {
          Ptr<ComponentCarrierBaseStation> c = i.second;
          ccPhyConfMap3.insert (std::make_pair (i.first,c));
        }

      //ComponentCarrierManager SAP
      rrc3->SetLteCcmRrcSapProvider (ccmEnbManager3->GetLteCcmRrcSapProvider ());
      ccmEnbManager3->SetLteCcmRrcSapUser (rrc3->GetLteCcmRrcSapUser ());
      // Set number of component carriers. Note: eNB CCM would also set the
      // number of component carriers in eNB RRC

      ccmEnbManager3->SetNumberOfComponentCarriers (ccMap3.size ());
      rrc3->ConfigureCarriers (ccPhyConfMap3);

      if (useIdealRrc)
        {
          Ptr<NrGnbRrcProtocolIdeal> rrcProtocol = CreateObject<NrGnbRrcProtocolIdeal> ();
          rrcProtocol->SetLteEnbRrcSapProvider (rrc3->GetLteEnbRrcSapProvider ());
          rrc3->SetLteEnbRrcSapUser (rrcProtocol->GetLteEnbRrcSapUser ());
          rrc3->AggregateObject (rrcProtocol);
        }
      else
        {
          Ptr<LteEnbRrcProtocolReal> rrcProtocol = CreateObject<LteEnbRrcProtocolReal> ();
      ...
      if (m_epcHelper != nullptr)
        {
          EnumValue epsBearerToRlcMapping;
          rrc3->GetAttribute ("EpsBearerToRlcMapping", epsBearerToRlcMapping);
          // it does not make sense to use RLC/SM when also using the EPC
          if (epsBearerToRlcMapping.Get () == LteEnbRrc::RLC_SM_ALWAYS)
            {
              rrc3->SetAttribute ("EpsBearerToRlcMapping", EnumValue (LteEnbRrc::RLC_UM_ALWAYS));
            }
        }
      ...
      // forward these calls to the specific MAC of some of the instances of component carriers. This
      // decision will depend on the specific implementation of the component carrier manager.
      rrc3->SetLteMacSapProvider (ccmEnbManager1->GetLteMacSapProvider ());
      rrc3->SetForwardUpCallback (MakeCallback (&NrGnbNetDevice::Receive, dev3));

      for (auto it = ccMap3.begin (); it != ccMap3.end (); ++it)
        {
          it->second->GetPhy ()->SetEnbCphySapUser (rrc3->GetLteEnbCphySapUser (it->first));
          rrc3->SetLteEnbCphySapProvider (it->second->GetPhy ()->GetEnbCphySapProvider (), it->first);

          rrc3->SetLteEnbCmacSapProvider (it->second->GetMac ()->GetEnbCmacSapProvider (),it->first );
          it->second->GetMac ()->SetEnbCmacSapUser (rrc3->GetLteEnbCmacSapUser (it->first));
          ...
          it->second->GetMac ()->SetLteCcmMacSapUser (ccmEnbManager3->GetLteCcmMacSapUser ());
          ccmEnbManager3->SetCcmMacSapProviders (it->first, it->second->GetMac ()->GetLteCcmMacSapProvider ());

          // insert the pointer to the LteMacSapProvider interface of the MAC layer of the specific component carrier
          ccmEnbManager3->SetMacSapProvider (it->first, it->second->GetMac ()->GetMacSapProvider ());
        }


      dev3->SetAttribute ("LteEnbComponentCarrierManager", PointerValue (ccmEnbManager3));
      dev3->SetCcMap (ccMap3);
      dev3->SetAttribute ("LteEnbRrc", PointerValue (rrc3));
      dev3->Initialize ();
      ...
      if (m_epcHelper != nullptr)
        {
          NS_LOG_INFO ("adding this eNB to the EPC");
          m_epcHelper->AddEnb (node, dev3, dev3->GetCellId ());
          Ptr<EpcEnbApplication> enbApp = node->GetApplication (0)->GetObject<EpcEnbApplication> ();
          NS_ASSERT_MSG (enbApp != nullptr, "cannot retrieve EpcEnbApplication");

          // S1 SAPs
          rrc3->SetS1SapProvider (enbApp->GetS1SapProvider ());
          enbApp->SetS1SapUser (rrc3->GetS1SapUser ());

          // X2 SAPs
          Ptr<EpcX2> x2 = node->GetObject<EpcX2> ();
          x2->SetEpcX2SapUser (rrc3->GetEpcX2SapUser ());
          rrc3->SetEpcX2SapProvider (x2->GetEpcX2SapProvider ());
        }
...
void NrHelper::ChangeGnbCam (const Ptr<Node> &node,
                                     const std::vector<std::reference_wrapper<BandwidthPartInfoPtr> > allBwps, std::string gnbCamType)
{
  Ptr<NetDevice> gnbDevice = node->GetDevice (0);
  SetGnbChannelAccessManagerTypeId (TypeId::LookupByName(gnbCamType));
  
  for (uint32_t bwpId = 0; bwpId < allBwps.size (); ++bwpId)
  {
    Ptr<NrGnbPhy> phy = GetGnbPhy (gnbDevice, bwpId);
    Ptr<NrChAccessManager> cam = DynamicCast<NrChAccessManager> (m_gnbChannelAccessManagerFactory.Create ());
    cam->SetNrSpectrumPhy (phy->GetSpectrumPhy ());
    phy->SetCam (cam);
  }
}
