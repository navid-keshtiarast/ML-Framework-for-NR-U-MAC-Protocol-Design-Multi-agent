...
NrMacSchedulerTdmaPF::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrMacSchedulerTdmaPF")
    ...
    .AddAttribute ("McsValue",
                   "Mcs value for adaptive coding modulation (ACM) scheme",
                   UintegerValue (1),
                   MakeUintegerAccessor(&NrMacSchedulerTdmaPF::SetMcsValue),
...
//21.11.2023
void
NrMacSchedulerTdmaPF::SetMcsValue (uint8_t mcs)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG ("TEEEEESTING MCS BUG IN FUNCTION");
  NrMacSchedulerNs3::SetStartMcsDl (mcs);
  NrMacSchedulerNs3::SetMaxDlMcs (mcs);
  NrMacSchedulerNs3::SetFixedDlMcs(true);
}
