...
void 
NrSpectrumPhy::fuckoff() const
{
  // Ptr<SpectrumSignalParameters> params=CreateObject<SpectrumSignalParameters>();
  // StartRx(params->psd);
  //// Time mo {MilliSeconds (8)};
  //// NS_LOG_FUNCTION (this);
  //// m_channelOccupied(mo);
  // std::cout<<"hihihihi";
  //ChangeState (, Time duration);
}
void
...
{
  m_channel = c;
  std::cout<<"jon khodetttt"<<m_channel;
...
NrSpectrumPhy::SetCcaMode1Threshold (double thresholdDBm)
{
  ...
  // convert dBm to Watt
  //std::cout<<"threshold"<<thresholdDBm;
...
void
NrSpectrumPhy::StartTxDataFrames_navid (const Ptr<PacketBurst>& pb, const std::list<Ptr<NrControlMessage> >& ctrlMsgList,
                                      Time duration)
{
  NS_LOG_FUNCTION (this);
  switch (m_state)
...
bool
NrSpectrumPhy::IsStateIdle (void)
{
  if (m_state == IDLE)
	  return true;
  else
...
NrSpectrumPhy::MaybeCcaBusy ()
{
  ...
  //Time bagh_bagho=m_interferenceCtrl->GetEnergyDuration (m_ccaMode1ThresholdW);
  //delayUntilCcaEnd+=m_interferenceSrs->GetEnergyDuration (m_ccaMode1ThresholdW);
  if (!delayUntilCcaEnd.IsZero ())
    {
      NS_LOG_DEBUG ("Channel detected BUSY for:" << delayUntilCcaEnd << " ns.");
      //std::cout<<"delayUntilCcaEnd"<<delayUntilCcaEnd;
  ...
  // else if (!bagh_bagho.IsZero ())
  // {
  //     NS_LOG_DEBUG ("Channel detected BUSY for:" << bagh_bagho << " ns.");
  //     //std::cout<<"delayUntilCcaEnd"<<delayUntilCcaEnd;
  //     ChangeState (CCA_BUSY, bagh_bagho);

  //     // check if with the new energy the channel will be for longer time in CCA_BUSY
  //     if ( m_busyTimeEnds < Simulator::Now() + bagh_bagho)
  //       {
  //         m_busyTimeEnds = Simulator::Now () + bagh_bagho;
  ...
  //         NS_LOG_DEBUG ("Check if still BUSY in:" << bagh_bagho << " us, and that is at "
  //             " time:"<<Simulator::Now() + bagh_bagho<<" and current time is:"<<Simulator::Now());

  //         m_checkIfIsIdleEvent = Simulator::Schedule (bagh_bagho, &NrSpectrumPhy::CheckIfStillBusy, this);
  //       }
  //   }
  // else if (Simulator::Now()>Seconds(0.6)&&IsEnb ())
  // {
    
  //   Time bagh_bagho=m_interferenceSrs->GetEnergyDuration (m_ccaMode1ThresholdW);
  //   if (!bagh_bagho.IsZero ())
  // {
  //     NS_LOG_DEBUG ("Channel detected BUSY for:" << bagh_bagho << " ns.");
  //     //std::cout<<"delayUntilCcaEnd"<<delayUntilCcaEnd;
  //     ChangeState (CCA_BUSY, bagh_bagho);
