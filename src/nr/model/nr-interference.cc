...
NrInterference::IsChannelBusyNow (double energyW)
{
  ...
  //std::cout<<powerDbm;
  // std::cout<<10 * log10 (energyW * 1000);
...
// for (uint32_t i=0;i< (*m_allSignals).GetValuesN();i++)
// {
//   NS_LOG_INFO("kekemal"<< m_allSignals->ValuesAt(i)<<' ');
// }    
  //tak takesh ra baressi kon dige integral nagir shayad farji shod
  ...
  if (detectedPowerW > energyW)
    {
      ...
      //std::cout<<"are";
      //NS_LOG_INFO ("detectedPowerW:"<<powerDbm);
      return true;
