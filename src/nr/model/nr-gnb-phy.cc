...
NrGnbPhy::StartSlot (const SfnSf &startSlot)
{
  ...
  id_of_canceling2=Simulator::Schedule (GetSlotPeriod (), &NrGnbPhy::EndSlot, this);
...
void
NrGnbPhy::StartSlot2 (const SfnSf &startSlot)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_channelStatus != TO_LOSE);
...
NrGnbPhy::EndSlot (void)
{
  ...
  m_currentSlot.Add (1);
  id_of_canceling=Simulator::Schedule (slotStart, &NrGnbPhy::StartSlot, this, m_currentSlot);
}

void
NrGnbPhy::EndSlot2 (void)
{
  NS_LOG_FUNCTION (this);
...
void
NrGnbPhy::DoSendReservationSignal ()

{
NS_LOG_FUNCTION (this) ;
//SetSubChannels (FromRBGBitmaskToRBAssignment (m_rbgAllocationPerSym.at (dci->m_symStart)));

if (!m_spectrumPhy->IsStateIdle())
  {
    NS_LOG_WARN ("Tried to send reservation signal when channel state is not IDLE.");
    return;
  }
  std::list<Ptr<NrControlMessage> > ctrlMsgs;
  m_spectrumPhy->StartTxDataFrames_navid (0, ctrlMsgs, m_lastSlotStart + GetSlotPeriod () - Simulator::Now ()- NanoSeconds(1));
...
NrGnbPhy::ChannelAccessGranted (const Time &time)
{
  ...
  Simulator::Schedule (MicroSeconds(1), &NrGnbPhy::DoSendReservationSignal, this);
  //SfnSf m_currentSlot2=m_currentSlot;
  // id_of_canceling.Cancel();
  // id_of_canceling2.Cancel();
  //ScheduleStartEventLoop(m_nodeid,0,0,0);
  //Simulator::Schedule (GetSlotPeriod (), &NrGnbPhy::EndSlot, this);
  //  for(int i=0; i<4;i++)
  // {
  // //   //m_currentSlot.Add(i);
  // //   m_currentSlot2.Add (1);
  // //   Simulator::Schedule (MicroSeconds(0)+i*GetSlotPeriod (), &NrGnbPhy::StartSlot2, this, m_currentSlot2);
  // Simulator::Schedule (MicroSeconds(71.427)+i*GetSlotPeriod (), &NrGnbPhy::CallMacForSlotIndication, this,m_currentSlot);
  // Simulator::Schedule (MicroSeconds(71.427)+i*GetSlotPeriod (), &NrGnbPhy::DoStartSlot, this);
  // //   Simulator::Schedule (MicroSeconds(0)+i*GetSlotPeriod ()+GetSlotPeriod (), &NrGnbPhy::EndSlot2, this);
  //  }
  //CallMacForSlotIndication (m_currentSlot);
  //DoStartSlot ();
  //m_currentSlot.Add(1);
  ...
  //id_of_canceling2.Cancel();
  //SfnSf m_currentSlot;
  //Simulator::ScheduleNow( &NrGnbPhy::StartSlot, this, m_currentSlot);
  //Simulator::Schedule (GetSlotPeriod (), &NrGnbPhy::EndSlot, this);
  ///m_currentSlot.Add ();
  
  //Simulator::ScheduleNow (&NrGnbPhy::DoStartSlot, this);
  //EndSlot();
  //id_of_canceling2.Cancel();

  //Simulator::Schedule (GetSlotPeriod (), &NrGnbPhy::EndSlot, this);
  //id_of_canceling.Cancel();
  //Simulator::Schedule (Simulator::Now, &NrGnbPhy::StartSlot, this, m_currentSlot);
  //m_currentSlot.Add (1);
  //Simulator::ScheduleNow( &NrGnbPhy::StartSlot, this, m_currentSlot);
  //m_currentSlot.Add (1);
  //StartSlot(m_currentSlot);
  //DoStartSlot ();
  // m_currentSlot.Add (1);
  // StartSlot(m_currentSlot);
  // Simulator::Schedule (GetSlotPeriod (), &NrGnbPhy::EndSlot, this);
  //m_lastSlotStart=m_lastSlotStart - GetSlotPeriod ();
  //EndSlot();
  //id_of_canceling.Cancel();
  //id_of_canceling.Cancel();
  //m_currentSlot.Add (1);
  //CallMacForSlotIndication (m_currentSlot);
  //DoStartSlot ();
  //Time slotStart = Simulator::Now ()+NanoSeconds(1);
  // id_of_canceling.Cancel();
  // id_of_canceling2.Cancel();
  //Simulator::Schedule (slotStart, &NrGnbPhy::EndSlot(), this);
  //m_currentSlot.Add (1);
  //StartSlot(m_currentSlot);
  ...
  NS_LOG_INFO("ridiraft ");
  }
  //navid
  // if(Simulator::Now()>Seconds(0.56))
  // {
    //m_lastSlotStart=m_lastSlotStart-MilliSeconds(1);
  // SfnSf startSlot (0, 0, 0, GetNumerology ());
  // StartSlot (startSlot);
  //}                       
}
