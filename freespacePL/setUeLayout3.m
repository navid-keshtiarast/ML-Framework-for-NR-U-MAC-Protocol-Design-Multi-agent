...
%   Detailed explanation goes here
    ...
    while counter < ueNums+1
        ...
        if bitGen == 1
            ...
            randomX = (ueLayoutMatrix(counter-1,1)-1) + 2*randi([0 100],1,1);
            randomY = signum*(ueLayoutMatrix(counter-1,2)-1) + 2*randi([0 100],1,1);

            if randomX < 0 || randomX > factorySizeVector(1)
                if randomX < 0
                    indReset = find(ueLayoutMatrix(:,1) == max(ueLayoutMatrix(1:counter-1,1)));
                    indReset = indReset(1,1);
                    randomX = (ueLayoutMatrix(indReset,1)-1) + 2*randi([0 100],1,1);
                    randomY = (ueLayoutMatrix(indReset,2)-1) + signum*2*randi([0 100],1,1);
                else
            ...
            elseif randomY < 0 || randomY > factorySizeVector(2)
                if randomY < 0
                    ...
                    indReset = indReset(1,1);
                    randomY = (ueLayoutMatrix(indReset,2)-1) + 2*randi([0 100],1,1);
                    randomX = (ueLayoutMatrix(indReset,1)-1) + signum*2*randi([0 100],1,1);
                else
