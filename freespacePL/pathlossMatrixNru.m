...
[gNbLayoutMatrix] = setGnbLayoutNru(factoryScenario,gNbDistance,gNbNums);
...
%% Create output files scenario

    [factorySizeVector,gNbDistance] = setFactoryLayout(factorySize,roomHeight);

    outputPath = '/home/navid/NS3_AI_RECMAC/ns-allinone-3.38/ns-3.38/freespacePL/Scenario1';
    [gNbLayoutMatrix] = setGnbLayoutNru(factoryScenario,gNbDistance,gNbNums);
...
%% Create output files scenario (Industrial)

    [factorySizeVector,gNbDistance] = setFactoryLayout(factorySize,roomHeight);

    outputPath = '/home/navid/NS3_AI_RECMAC/ns-allinone-3.38/ns-3.38/freespacePL/Scenario2';
    ...
    %% Create output files scenario (Close)

    [factorySizeVector,gNbDistance] = setFactoryLayout(factorySize,roomHeight);

    outputPath = '/home/inets/Desktop/A/ns-allinone-3.38/ns-3.38/freespacePL/Scenario4';
    [gNbLayoutMatrix] = setGnbLayoutNru(factoryScenario,gNbDistance,gNbNums);
    [pathlossMatrixGnb,gNbNums,~] = calculatePathloss(gNbLayoutMatrix, gNbLayoutMatrix, carrierFrequency, onlyLOS, factoryScenario);
    writematrix(gNbLayoutMatrix,append(outputPath,'/gNbLayoutMatrix.txt'),'Delimiter','space');
    writematrix(pathlossMatrixGnb,append(outputPath,'/pathlossMatrixGnbGnb.txt'),'Delimiter','space');
